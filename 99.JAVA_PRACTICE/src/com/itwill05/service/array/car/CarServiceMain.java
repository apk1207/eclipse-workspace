...
public class CarServiceMain {
	public static void main(String[] args) {
		...
		int totNumber = carService.getTotalParkingNumber();
		System.out.println(">>> "+totNumber+"구역");
		
		System.out.println("3.CarService객체야 주차가능주차구획수반환해줘");
		int availableNuber = carService.getAvailableParkingNumber();
		System.out.println(">>> "+availableNuber+"구역");
		
		System.out.println("4.CarService객체야 차량번호(2322번) 인자로줄께  차객체한대 참조변수반환해줘");
		Car findByNoCar = carService.findByNo("2322");
		System.out.printf(">>>  ");
		findByNoCar.print();
		
		System.out.println("5.CarService객체야 입차시간(8시이후) 인자로줄께 차객체배열 참조변수반환해줘");
		Car[] findByIntimeCars = carService.findByInTime(8);
		for (Car intimeCar : findByIntimeCars) {
			intimeCar.print();
		}
		
		
		System.out.println("-------------출차 전----------");
		carService.print();
