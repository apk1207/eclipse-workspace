	...
	public StudentServiceArrayListFrameMain() {
		...
		findByNoStudentBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int inputNo = Integer.parseInt(noTF.getText());
		...
		findByGradeStudentsBtn.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(gradeTF.getText().equals("")) {
					gradeTF.requestFocus();
					return;
				}
				char inputGrade = gradeTF.getText().charAt(0);
		...
		findByNameStudentsBtn = new JButton("6.  이름 입력하면 학생들 반환");
